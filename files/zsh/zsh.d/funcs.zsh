...
function gpg_export_pubkey () {
    gpg --output public.key --export $1
}
...
function gpg_export_privkey () {
    gpg --output private.key --export-secret-subkeys $1
}
...
function gpg_print_pubkey () {
    gpg --armor --export $1 | tee pubkey.asc
...
function digga() {
    dig +nocmd +multiline +noall +answer ${2:-A} ${1};
}
...
function proxy_ssh {
    ...
    # opens a proxy tunnel at local 8443 to ssh alias. Requires socks5 local browser configuration (firefox)
    ssh -C2qTnN -D 8443 $1
}
